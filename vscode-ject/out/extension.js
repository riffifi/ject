...
const builtinSignatures = new Map([
    // Type & conversion
...
let statusBarItem;
let checkTimeout;
let moduleMembersCache;
let moduleMembersCacheRoot;
const savePipelineGuard = new Set();
// ─── Activate ────────────────────────────────────────────────────────────────
function activate(context) {
    ...
    // Commands
    vscode.commands.registerCommand("ject.runFile", runCurrentFile), vscode.commands.registerCommand("ject.runSelection", runSelection), vscode.commands.registerCommand("ject.startRepl", startRepl), vscode.commands.registerCommand("ject.buildInterpreter", buildInterpreter), vscode.commands.registerCommand("ject.checkFile", checkCurrentFile), vscode.commands.registerCommand("ject.openExamples", openExamples), vscode.commands.registerCommand("ject.newFile", newJectFile), vscode.commands.registerCommand("ject.organizeImports", organizeImportsCommand), vscode.commands.registerCommand("ject.formatFile", formatCurrentFile), 
    // Language providers
    vscode.languages.registerCompletionItemProvider(languageSelector, new JectCompletionProvider(), ".", "\"", "("), vscode.languages.registerHoverProvider(languageSelector, new JectHoverProvider()), vscode.languages.registerDocumentSymbolProvider(languageSelector, new JectDocumentSymbolProvider()), vscode.languages.registerWorkspaceSymbolProvider(new JectWorkspaceSymbolProvider()), vscode.languages.registerDocumentFormattingEditProvider(languageSelector, new JectFormattingProvider()), vscode.languages.registerSignatureHelpProvider(languageSelector, new JectSignatureHelpProvider(), "(", ","), vscode.languages.registerDefinitionProvider(languageSelector, new JectDefinitionProvider()), vscode.languages.registerReferenceProvider(languageSelector, new JectReferenceProvider()), vscode.languages.registerRenameProvider(languageSelector, new JectRenameProvider()), vscode.languages.registerFoldingRangeProvider(languageSelector, new JectFoldingProvider()), vscode.languages.registerCodeActionsProvider(languageSelector, new JectCodeActionProvider(), {
        providedCodeActionKinds: [vscode.CodeActionKind.QuickFix, vscode.CodeActionKind.RefactorExtract]
    }), 
    // Events
    vscode.window.onDidChangeActiveTextEditor(updateStatusBar), vscode.workspace.onDidChangeTextDocument(onDocumentChange), vscode.workspace.onDidSaveTextDocument((doc) => { void onDocumentSave(doc); }), vscode.workspace.onDidCloseTextDocument((doc) => diagnosticCollection.delete(doc.uri)), vscode.window.onDidCloseTerminal((t) => {
        if (t === runTerminal) {
    ...
    (0, testing_1.registerJectTesting)(context);
}
function deactivate() {
...
function onDocumentChange(event) {
    if (event.document.languageId !== "ject") {
        return;
    }
    if (!vscode.workspace.getConfiguration("ject").get("lintOnType", true)) {
        return;
    }
    if (checkTimeout) {
...
async function onDocumentSave(document) {
    ...
    const shouldFormat = cfg.get("autoFormatOnSave", false);
    const shouldOrganize = cfg.get("organizeImportsOnSave", false);
...
async function lintDocument(document) {
    ...
    const root = (0, jectExec_1.workspaceRoot)();
...
async function runCurrentFile(uri) {
    ...
    terminal.show(true);
    terminal.sendText(`${(0, jectExec_1.quoteShellArg)(executable)} ${(0, jectExec_1.quoteShellArg)(document.uri.fsPath)}`);
}
async function runSelection() {
    ...
    const terminal = getRunTerminal();
    terminal.show(true);
    terminal.sendText(`${(0, jectExec_1.quoteShellArg)(executable)} ${(0, jectExec_1.quoteShellArg)(tmpFile)}`);
}
async function startRepl() {
    const executable = await (0, jectExec_1.resolveJectExecutable)();
    replTerminal = replTerminal ?? vscode.window.createTerminal({ name: "Ject REPL", cwd: (0, jectExec_1.workspaceRoot)() });
    replTerminal.show(true);
    replTerminal.sendText((0, jectExec_1.quoteShellArg)(executable));
}
...
async function checkCurrentFile() {
    ...
    childProcess.execFile(checkCmd.cmd, checkCmd.args, { cwd: checkCmd.cwd, timeout: 15000 }, (error, stdout, stderr) => {
        ...
        if (diagnostics.length === 0 && !error) {
            ...
            getRunTerminal().sendText(`${(0, jectExec_1.quoteShellArg)(checkCmd.cmd)} ${checkCmd.args.map(jectExec_1.quoteShellArg).join(" ")}`);
        }
        else {
...
async function organizeImportsCommand() {
    const editor = vscode.window.activeTextEditor;
    if (!editor || editor.document.languageId !== "ject") {
        vscode.window.showWarningMessage("Open a Ject file first.");
        return;
    ...
    provideCompletionItems(document, position) {
        ...
        for (const [moduleName, members] of Object.entries(allModuleMembers)) {
            for (const member of members) {
                if (builtinSignatures.has(member)) {
                    continue;
                }
...
class JectHoverProvider {
    provideHover(document, position) {
        ...
        if (word === "PI") {
            return new vscode.Hover(new vscode.MarkdownString("`PI` = 3.141592653589793"), range);
        }
        if (word === "E") {
            return new vscode.Hover(new vscode.MarkdownString("`E` = 2.718281828459045"), range);
        }
...
// ─── Signature help ──────────────────────────────────────────────────────────
class JectSignatureHelpProvider {
...
// ─── Rename ──────────────────────────────────────────────────────────────────
class JectRenameProvider {
    prepareRename(document, position) {
        const range = document.getWordRangeAtPosition(position);
        const word = range ? document.getText(range) : undefined;
        if (!word || keywords.includes(word) || builtins.includes(word)) {
            return undefined;
        }
        return range;
    }
...
// ─── Workspace symbols ───────────────────────────────────────────────────────
class JectWorkspaceSymbolProvider {
...
class JectCodeActionProvider {
    provideCodeActions(document, range, context) {
        ...
        const allModuleMembers = getResolvedModuleMembers();
        const missingForBatch = new Map();
        for (const diagnostic of context.diagnostics) {
            const symbol = extractUndefinedSymbol(diagnostic.message);
            if (!symbol) {
                continue;
            }
            if (isAlreadyKnownSymbol(document, symbol)) {
                continue;
            }
...
function formatJect(source, indentSize) {
    ...
    for (const rawLine of lines) {
        ...
        if (/^(end|elseif\b|else\b|catch\b)/.test(trimmed)) {
...
function collectDocumentSymbols(document) {
    const result = { functions: [], variables: [], structs: [] };
    for (let line = 0; line < document.lineCount; line++) {
        const trimmed = document.lineAt(line).text.trim();
        ...
        if (letMatch) {
            result.variables.push({ name: letMatch[1], line });
        }
        if (structMatch) {
            result.structs.push({ name: structMatch[1], line });
        }
    }
    return result;
}
/** Resolve `import "X" as alias` → returns the module name for the given alias. */
function resolveAlias(document, alias) {
    for (let line = 0; line < document.lineCount; line++) {
        const trimmed = document.lineAt(line).text.trim();
...
function getResolvedModuleMembers() {
    const root = (0, jectExec_1.workspaceRoot)();
    if (!root) {
...
function buildModuleMembersFromStdlib(root) {
    const merged = {};
    ...
    for (const entry of fs.readdirSync(stdlibDir)) {
        ...
        for (const line of lines) {
            ...
            if (fnMatch) {
                bucket.add(fnMatch[1]);
            }
            if (valueMatch) {
                bucket.add(valueMatch[1]);
            }
        }
...
function buildAutoImportEdit(document, moduleName, symbol) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (firstImport !== -1 && lastImport !== -1 && i > lastImport) {
            break;
        }
    }
    for (let i = 0; i < lines.length; i++) {
...
function buildBatchAutoImportEdit(document, symbolsToModules) {
    const lines = document.getText().replace(/\r\n/g, "\n").split("\n");
    ...
    const selective = new Map();
    const fullOrAlias = new Set();
    const importLines = [];
    for (let i = 0; i < lines.length; i++) {
        const trimmed = lines[i].trim();
        if (!trimmed.startsWith("import ")) {
        ...
        if (s) {
            ...
            const existing = selective.get(moduleName) ?? new Set();
    ...
    for (const [symbol, moduleName] of symbolsToModules) {
        if (fullOrAlias.has(moduleName)) {
            continue;
        }
        const set = selective.get(moduleName) ?? new Set();
...
function extractUndefinedSymbol(message) {
    const m = message.match(/(?:undefined variable|undeclared variable)\s+`([A-Za-z_][A-Za-z0-9_]*)`/i);
    return m?.[1];
}
function findModuleForSymbol(symbol, membersByModule) {
...
function isAlreadyKnownSymbol(document, symbol) {
    if (builtinSignatures.has(symbol)) {
        return true;
    }
    ...
    if (syms.functions.some(f => f.name === symbol)) {
        return true;
    }
    if (syms.variables.some(v => v.name === symbol)) {
        return true;
    }
    if (syms.structs.some(s => s.name === symbol)) {
        return true;
    }
    return false;
}
async function organizeImportsDocument(document) {
    ...
    const usedText = lines
        ...
        .join("\n");
    const usedWord = (name) => new RegExp(`\\b${escapeRegExp(name)}\\b`).test(usedText);
    const normalized = [];
...
async function getTargetDocument(uri) {
    if (uri) {
        ...
        if (doc.languageId === "ject" || [".ject", ".jt"].includes(path.extname(doc.uri.fsPath))) {
            return doc;
        }
    }
    const editor = vscode.window.activeTextEditor;
...
function getRunTerminal() {
    runTerminal = runTerminal ?? vscode.window.createTerminal({ name: "Ject", cwd: (0, jectExec_1.workspaceRoot)() });
    return runTerminal;
}
