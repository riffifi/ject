...
impl crate::native::NativeObject for NdArray {
...
/// The jnum native module: wraps `call_jnum_function` (which keeps its
/// existing internal `"np_"`-prefixed dispatch table unchanged) behind the
/// generalized `NativeModule` interface, so the interpreter no longer needs to
/// know jnum exists or sniff a naming convention to route calls to it.
pub struct JnumModule;

impl crate::native::NativeModule for JnumModule {
    ...
    fn call(&self, fn_name: &str, args: Vec<Value>) -> Result<Value, RuntimeError> {
        let prefixed = format!("np_{}", fn_name);
        call_jnum_function(&prefixed, args)
    }
}

/// Wraps an `NdArray` as a `Value`.
pub fn ndarray_value(arr: NdArray) -> Value {
    Value::Native(crate::native::NativeValue::new(arr))
}

/// Extracts an `&NdArray` from a `Value`, if it holds one.
pub fn as_ndarray(value: &Value) -> Option<&NdArray> {
    match value {
        Value::Native(native) => native.downcast_ref::<NdArray>(),
        _ => None,
    }
}
...
/// Create jnum module
pub fn create_jnum_module() -> HashMap<String, Value> {
    ...
    module.insert(
        ...
        Value::NativeFunction {
            module: "jnum".to_string(),
            name: "lexsort".to_string(),
        },
    );
    module.insert(
        "searchsorted".to_string(),
        Value::NativeFunction {
            module: "jnum".to_string(),
            name: "searchsorted".to_string(),
        },
    );
    module.insert(
        "nonzero".to_string(),
    ...
    module.insert(
        "intersect1d".to_string(),
        Value::NativeFunction {
            module: "jnum".to_string(),
            name: "intersect1d".to_string(),
        },
    );
    module.insert(
        "union1d".to_string(),
    ...
    module.insert(
        ...
        Value::NativeFunction {
            module: "jnum".to_string(),
            name: "setxor1d".to_string(),
        },
    );

    // Logical operations
    module.insert(
        "logical_and".to_string(),
        Value::NativeFunction {
            module: "jnum".to_string(),
    ...
    module.insert(
        "logical_xor".to_string(),
        Value::NativeFunction {
            module: "jnum".to_string(),
            name: "logical_xor".to_string(),
        },
    );
    module.insert(
        "greater".to_string(),
    ...
    // Uppercase aliases matching jnum's conventional np.NAN / np.INF naming.
    module.insert("NAN".to_string(), Value::Float(f64::NAN));
    module.insert("INF".to_string(), Value::Float(f64::INFINITY));
...
/// Call jnum builtin function
pub fn call_jnum_function(name: &str, args: Vec<Value>) -> Result<Value, RuntimeError> {
    match name {
        // Array creation
        ...
        _ => Err(RuntimeError {
            message: format!("Unknown jnum function: {}", name),
        }),
    }
}
...
fn np_array(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    match &args[0] {
        Value::Array(arr) => {
            ...
            Ok(crate::jnum::ndarray_value(ndarray))
        }
        _ => Err(RuntimeError {
...
fn np_full(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    let shape = match &args[0] {
        Value::Array(shape_arr) => shape_arr
            .borrow()
            .iter()
            .filter_map(|v| match v {
                Value::Integer(i) => Some(*i as usize),
...
fn np_shape(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    if let Some(arr) = crate::jnum::as_ndarray(&args[0]) {
        let shape: Vec<Value> = arr
            .shape()
            .iter()
            .map(|&d| Value::Integer(d as i64))
            .collect();
        return Ok(Value::array(shape));
    }
    match &args[0] {
        Value::Array(arr) => Ok(Value::array(vec![
            Value::Integer(arr.borrow().len() as i64),
        ])),
        _ => Err(RuntimeError {
            message: "shape() requires an array argument".to_string(),
        }),
...
fn np_ndim(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    if let Some(arr) = crate::jnum::as_ndarray(&args[0]) {
        return Ok(Value::Integer(arr.ndim() as i64));
    }
    match &args[0] {
...
fn np_dtype(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    match crate::jnum::as_ndarray(&args[0]).cloned() {
        Some(arr) => {
            let dtype = match arr {
...
fn np_squeeze(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    match crate::jnum::as_ndarray(&args[0]).cloned() {
        Some(arr) => {
            ...
            let shape = if new_shape.is_empty() {
                vec![1]
            } else {
                new_shape
            };
    ...
    match &args[0] {
        Value::Array(arrays) => {
            ...
            for arr in arrays.borrow().iter() {
                match arr {
                    _ if crate::jnum::as_ndarray(arr).is_some() => {
                        let a = crate::jnum::as_ndarray(arr).unwrap();
            ...
            let ndarray =
                NdArray::F64(ArrayD::from_shape_vec(vec![all_data.len()], all_data).unwrap());
            Ok(crate::jnum::ndarray_value(ndarray))
        }
        _ => Err(RuntimeError {
...
fn np_split(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    match crate::jnum::as_ndarray(&args[0]).cloned() {
        Some(arr) => {
            ...
            let result = vec![crate::jnum::ndarray_value(arr.clone())];
...
fn np_mean(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    if let Some(arr) = crate::jnum::as_ndarray(&args[0]) {
        let data = arr.to_f64_vec()?;
        let sum: f64 = data.iter().sum();
        return Ok(Value::Float(sum / data.len() as f64));
    }
    match &args[0] {
        Value::Array(arr) => {
            let arr = arr.borrow();
...
fn np_dot(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    match (
        crate::jnum::as_ndarray(&args[0]).cloned(),
        crate::jnum::as_ndarray(&args[1]).cloned(),
    ) {
        (Some(a), Some(b)) => {
...
fn np_unique(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    match crate::jnum::as_ndarray(&args[0]).cloned() {
        Some(arr) => {
            ...
            let ndarray =
                NdArray::F64(ArrayD::from_shape_vec(vec![unique_data.len()], unique_data).unwrap());
            Ok(crate::jnum::ndarray_value(ndarray))
        }
        _ => Err(RuntimeError {
...
fn np_logical_and(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    element_wise_op(&args[0], &args[1], |a, b| {
        if a != 0.0 && b != 0.0 {
            1.0
        } else {
            0.0
...
where
    F: Fn(f64) -> f64,
{
    match crate::jnum::as_ndarray(arr) {
        Some(a) => {
...
where
    F: Fn(f64, f64) -> f64,
{
    match (crate::jnum::as_ndarray(a), crate::jnum::as_ndarray(b)) {
        (Some(arr_a), Some(arr_b)) => {
            ...
            if data_a.len() == 1 {
                ...
                let ndarray =
                    NdArray::F64(ArrayD::from_shape_vec(vec![result.len()], result).unwrap());
                return Ok(crate::jnum::ndarray_value(ndarray));
            }
            if data_b.len() == 1 {
                let scalar = data_b[0];
        ...
        _ if crate::jnum::as_ndarray(a).is_some() || crate::jnum::as_ndarray(b).is_some() => {
            let (arr, scalar) = if let Some(nd) = crate::jnum::as_ndarray(a).cloned() {
                (nd, b)
            } else {
                (crate::jnum::as_ndarray(b).cloned().unwrap(), a)
            };
